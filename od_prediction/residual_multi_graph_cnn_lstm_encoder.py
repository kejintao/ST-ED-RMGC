...
from keras.layers import Dense, Activation, Dropout, Lambda, Flatten, Reshape, Permute, \
    LSTM, Concatenate, Add, BatchNormalization
...
def run_model():
    ...
    graph_encoded = convolution_block(X_input, graph_conv_filters_input, units, num_filters)
    graph_encoded = identity_block(graph_encoded, graph_conv_filters_input, units, num_filters)
    ...
    for l in range(1, len(NETWORK_STRUCTURE_E)):
        ...
        graph_encoded = convolution_block(graph_encoded, graph_conv_filters_input, units, num_filters)
        graph_encoded = identity_block(graph_encoded, graph_conv_filters_input, units, num_filters)
        # graph_encoded = Dropout(0.2)(graph_encoded)
    ...
    # sgd = RMSprop(lr=LR_RATE, rho=0.9, epsilon=None, decay=0.0)
    ...
    save_file_name = CITY + "_" + str(SAMPLE_CASE) + "\\residual_multi_graph_cnn_lstm_ed---" + str(int(LR_RATE * 10000)) + "-"
...
if __name__ == "__main__":
    ...
    NETWORK_STRUCTURE_L = [128, 64]
    LR_RATE = 0.0003
    REGULARIZER_RATE = 0
    REGULARIZER = l2(REGULARIZER_RATE)
    ACTIVATION = 'relu'
    LATENT_DIM = 90
    LATENT_DIM_L = 30
